...
const Home = () => {
  ...
  const [selectedIngredients, setSelectedIngredients] = useState([]); // Still an array, but allows duplicates
  ...
  const toggleIngredient = useCallback((item) => {
    setSelectedIngredients(prev => {
      ...
      if (countInSelection < maxAllowed) {
        return [...prev, item]; // Add another instance
      } else {
        return prev.filter((i, idx) => i !== item || prev.indexOf(i) !== idx); // Remove one instance
      }
    });
  }, [player.inventory])
